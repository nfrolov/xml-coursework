...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/music">
        <html>
            <head>
                <title>List by artist name</title>
                <style type="text/css">
            ...
            <body>
                <h1>List by artist name</h1>
                <xsl:apply-templates select="compilation/artist[generate-id() = generate-id(key('artists', .)[1])]">
                    <xsl:sort select="." />
                </xsl:apply-templates>
            </body>
        </html>
    ...
    <xsl:template match="artist">
        <xsl:variable name="current-artist" select="." />
        <div class="artist">
            <h2><xsl:value-of select="$current-artist" /></h2>
            <xsl:for-each select="key('artists', $current-artist)">
